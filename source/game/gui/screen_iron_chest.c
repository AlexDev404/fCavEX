...
static uint8_t iron_chest_container;

void screen_iron_chest_set_windowc(uint8_t container) {
	iron_chest_container = container;
}
...
static void screen_iron_chest_update(struct screen* s, float dt) {
	if(input_pressed(IB_INVENTORY)) {
		svin_rpc_send(&(struct server_rpc) {
			...
			.payload.window_close.window = iron_chest_container,
		});
	...
	if(input_pressed(IB_GUI_CLICK)) {
		...
		if(windowc_new_action(gstate.windows[iron_chest_container], &action_id,
			...
			svin_rpc_send(&(struct server_rpc) {
				...
				.payload.window_click.window = iron_chest_container,
